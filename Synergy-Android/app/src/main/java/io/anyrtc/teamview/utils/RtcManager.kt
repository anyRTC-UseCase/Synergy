...
class RtcManager private constructor(){
    ...
    fun initRtc(context: Context/*, rtcListener: IRtcEngineEventHandler*/) {
        rtcEngine = RtcEngine.create(
            context, SpUtil.get().getString(MyConstants.APP_ID, ""), rtmListener/*rtcListener*/
        )
    }
    ...
    fun initRtm(/*listener: RtmClientListener*/) {
        rtmClient = RtmClient.createInstance(
            ...
            SpUtil.get().getString(MyConstants.APP_ID, "").toString(),
    ...
    fun loginRtm(rtmToken: String, uid: String, loginFailedCallback: (filed: Boolean, var2: ErrorInfo?) -> Unit) {
        logoutRtm()
    ...
    fun sendCall(uid: String, content: String, sent: (success: bool, err: ErrorInfo?, localInvitation: LocalInvitation?) -> Unit) {
    ...
    fun cancelCall(localInvitation: LocalInvitation, callback: (success: bool, err: ErrorInfo?) -> Unit) {
    ...
    private class RtmListener constructor(private val callbackList: MutableList<RtcHandler?>) : RtmChannelListener, IRtcEngineEventHandler() {
    ...
    private class RtmCallListener(private val callbackList: MutableList<RtcHandler?>) : RtmCallEventListener {
