...
func CreateAccessToken2(appID, appCertificate, channelName string, uid string) AccessToken {
	...
	//return AccessToken{appID, appCertificate, channelName, uid, ts, salt, message, "", ""}
}

func (token *AccessToken) CompareToken(source AccessToken) (bool, error) {
	buf_m := new(bytes.Buffer)
	if err := packUint32(buf_m, source.Salt); err != nil {
		return false, err
	}
	//fmt.Println("-------------------------->Compare Token with source.Salt: ", source.Salt)
	//fmt.Println("-------------------------->Compare Token with source.Ts: ", source.Ts)

	if err := packUint32(buf_m, source.Ts); err != nil {
		return false, err
	}
	if err := packMapUint32(buf_m, source.Message); err != nil {
		return false, err
	}
	bytes_m := buf_m.Bytes()

	//var lenChannel = len(source.ChannelName)
	//source.ChannelName = source.ChannelName[:(lenChannel - len(source.MsgRawContent))]
	//var lenUid = len( source.UidStr)
	//source.UidStr = source.UidStr[:(lenUid - len(source.MsgRawContent))]

	buf_val := new(bytes.Buffer)
	val := source.AppID + source.ChannelName + source.UidStr
	buf_val.Write([]byte(val))
	buf_val.Write(bytes_m)
	bytes_val := buf_val.Bytes()

	//fmt.Println("-------------------------->Compare Token with buf_val: ", val)
	buf_sig := hmac.New(sha256.New, []byte(source.AppCertificate))
	buf_sig.Write(bytes_val)
	bytes_sig := buf_sig.Sum(nil)

	fmt.Println("-------------------------->Compare Token with bytes_sig: ",  base64.StdEncoding.EncodeToString(bytes_sig))
	return true, nil
}

func (token *AccessToken) VerifyToken(source AccessToken, sourceToken string) (bool, error) {
	buildToken, err := source.Build()
	if err != nil {
		return false, err
	}
	return sourceToken == buildToken, nil
}
...
func (token *AccessToken) FromString(originToken string) bool {
	...
	//signature_, chan_name_, uid_, msg_raw_content_, err := unPackContent(originContentDecoded)
	if err != nil {
		return false
	}
	...
	//var len_chan_name = len(chan_name_)
	//chan_name_ = chan_name_[:(len_chan_name - len(msg_raw_content_))]
	//token.ChannelName = chan_name_
	//
	//var lenUid = len(uid_)
	//uid_ =  uid_[:(lenUid - len(msg_raw_content_))]
	//token.UidStr = uid_
...
func (token *AccessToken) Build() (string, error) {
	...
	fmt.Println("-------------------------->Build Token with token.Ts: ", token.Ts)
	...
	fmt.Println("-------------------------->Build Token with buf_val: ", val)
	buf_sig := hmac.New(sha256.New, []byte(token.AppCertificate))
	buf_sig.Write(bytes_val)
	bytes_sig := buf_sig.Sum(nil)

	fmt.Println("-------------------------->Build Token with bytes_sig: ",  base64.StdEncoding.EncodeToString(bytes_sig))

	//buf_chanName := new(bytes.Buffer)
	//buf_chanName.Write([]byte(token.ChannelName))
	//buf_chanName.Write(bytes_m)
	//bytes_chanName := buf_chanName.Bytes()
	//
	//
	//buf_Uid := new(bytes.Buffer)
	//buf_Uid.Write([]byte(token.UidStr))
	//buf_Uid.Write(bytes_m)
	//bytes_Uid := buf_Uid.Bytes()
	...
	fmt.Println("-------------------------->Build Token crc_channel_name: ",  crc_channel_name)
	fmt.Println("-------------------------->Build Token with crc_uid: ",  crc_uid)
	...
	//if err := packString(buf_content, string(bytes_chanName[:])); err != nil {
	//	return ret, err
	//}
	//if err := packString(buf_content, string(bytes_Uid[:])); err != nil {
	//	return ret, err
	//}
	...
	fmt.Println("-------------------------->Build Token result: ",  ret)
	return ret, nil
}
...
//func unPackContent(buff []byte) (string, string, string, string, error) {
	...
	//chanName, err := unPackString(in)
	//if err != nil {
	//	//return "", 0, 0, "", err
	//	return "", "", "", "", err
	//}
	//uId, err := unPackString(in)
	//if err != nil {
	//	//return "", 0, 0, "", err
	//	return "", "", "", "", err
	//}
